...
echo -e "${DIM}OpenWA's own SSRF guard refuses to webhook to localhost, so an incoming message has nowhere to go until you"
echo -e "register a public URL for it - confirmed live: WhatsApp linking succeeds, the QR scan works, and every message"
echo -e "you send afterward just silently goes nowhere, with nothing in any log to point at why.${RESET}"
echo -e "${DIM}  1. Install ngrok if you don't have it: brew install ngrok"
echo -e "  2. One-time only: ngrok config add-authtoken <token from https://dashboard.ngrok.com>"
echo -e "  3. In a terminal you leave running the whole time you use Adiyan: ngrok http 8425"
echo -e "  4. Once mesh/start_all.sh has run and WhatsApp is linked: .venv/bin/python3 -m mesh.mcp.whatsapp.register_webhook${RESET}"
