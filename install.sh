...
echo -e "\n${RED}${BOLD}One more step before any WhatsApp message will actually reach Adiyan:${RESET}"
echo -e "${DIM}OpenWA's own SSRF guard refuses to webhook to localhost, so an incoming message has nowhere to go until"
echo -e "mesh/start_all.sh can register a public URL for it - confirmed live: WhatsApp linking succeeds, the QR scan"
echo -e "works, and every message you send afterward just silently goes nowhere, with nothing in any log to point at"
echo -e "why. start_all.sh runs ngrok and re-registers the webhook for you automatically on every run - the only part"
echo -e "it can't do on your behalf is the one-time ngrok account:"
echo -e "  1. Install ngrok if you don't have it: brew install ngrok"
echo -e "  2. One-time only: ngrok config add-authtoken <token from https://dashboard.ngrok.com>${RESET}"
echo -e "\n${DIM}Optional: sharing spare compute with another Adiyan user (mesh/compute_share/) is off the internet by"
echo -e "default (loopback-only) - nothing to do for a normal single-machine install. For a real cross-machine"
echo -e "test, both sides need to be reachable from each other (Tailscale is the simplest way - see the Peer"
echo -e "Exchange design doc) and, if you're also exposing this same machine's WhatsApp webhook via ngrok, a"
echo -e "SECOND, separate ngrok account for compute_share's own tunnel - confirmed live, one free ngrok account"
echo -e "only allows one tunnel online at a time, so the two would fight over the same slot otherwise.${RESET}"
