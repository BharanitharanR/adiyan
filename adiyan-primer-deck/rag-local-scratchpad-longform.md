...
**The correction that lost to the thing it corrected.** Someone updated a stated preference. The system kept both the old statement and the correction, and ranked the *stale* one higher — 0.781 against 0.725. Naive top-1 retrieval returns the wrong answer with total confidence. I assumed the cause was length, and a controlled ablation across three embedding models proved me wrong: the real culprit is that the correction *names the thing it corrects*, and dense embeddings do not represent negation.
