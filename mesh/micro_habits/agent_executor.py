...
class MicroHabitParams(BaseModel):
    entry: str = Field(description="The micro habit entry to log")
    mood: Optional[str] = Field(default=None, description="Optional mood description")


class ListMicroHabitsParams(BaseModel):
    limit: int = Field(default=20, description="How many recent entries to return. Default 20.")

EXTRACTION_SCHEMAS = {
    'log_micro_habit': MicroHabitParams,
    'list_micro_habits': ListMicroHabitsParams,
}

class MicroHabitAgentExecutor(AgentExecutor):
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        if skill_id not in EXTRACTION_SCHEMAS:
