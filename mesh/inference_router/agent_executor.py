"""
Inference Router's AgentExecutor. Same design as every other agent under
mesh/ - copied from mesh/journal/agent_executor.py, the smallest real one,
and adapted for this agent's one skill.

Everything in this file except the two lines that mention `complete` by
name is boilerplate every agent in this mesh shares verbatim: the A2A
task lifecycle, the DataPart fast-path (mesh/lib/agent_sdk.py's ask() is
the only real caller, and it always uses this path - the plain-language
routing fallback below exists for shape-consistency with every other
agent, not because a human message is ever expected to land here), and
...
from mesh.inference_router.skills import complete
...
class CompleteParams(BaseModel):
    caller_agent_id: str = Field(description="Which agent this completion is on behalf of.")
    stage: str = Field(default='default', description="Per-agent pipeline-step name, for dashboard-editable model/temperature.")
    prompt: str = Field(description="The fully-built prompt to complete.")
    model: str = Field(default='qwen3:8b-16k')
    temperature: float = Field(default=0.4)
    community: str = Field(default='', description="Pass 'communitySearch' to allow offloading to a peer if this machine is busy.")


EXTRACTION_SCHEMAS = {'complete': CompleteParams}
# --- end of what changes per-skill ---


class InferenceRouterExecutor(AgentExecutor):
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            result = await complete.run(**params)
        except Exception as e:
            await updater.failed(new_text_message(f'complete failed: {e}'))
            return
        # --- end of what changes per-skill ---
